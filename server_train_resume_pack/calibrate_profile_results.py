...
def maybe_write_gui_config(summary, backend_mode):
    ...
    threshold_key = key_by_backend[backend_mode]

    store = ConfigStore()
    config = store.load()
    config[threshold_key] = float(summary["threshold_far_1"])
...
def main():
    ...
    if args.write_gui_config:
        ...
        print(
            f"GUI config updated: {threshold_key}={summary['threshold_far_1']:.6f}"
        )
