...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional MUSAN root directory for additive waveform augmentation.",
    )
    parser.add_argument(
        "--musan-prob",
        type=float,
        default=0.0,
        help="Probability of applying one MUSAN augmentation per training sample.",
    )
    parser.add_argument(
        "--musan-snr-noise",
        type=str,
        default="0,15",
        help="Noise SNR range in dB as min,max.",
    )
    parser.add_argument(
        "--musan-snr-speech",
        type=str,
        default="13,20",
        help="Speech/babble SNR range in dB as min,max.",
    )
    parser.add_argument(
        "--musan-snr-music",
        type=str,
        default="5,15",
        help="Music SNR range in dB as min,max.",
    )
    parser.add_argument(
        "--rir-root",
        type=str,
        default="",
        help="Optional RIR root directory for waveform reverberation augmentation.",
    )
    parser.add_argument(
        "--reverb-prob",
        type=float,
        default=0.0,
        help="Probability of applying one RIR-based reverb augmentation per sample.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Run verification monitor every N epochs. 1 means every epoch.",
    )
...
def build_loaders(args, device):
    ...
    train_dataset = SpeakerDataset(
        ...
        musan_prob=args.musan_prob,
        musan_snr_noise=args.musan_snr_noise,
        musan_snr_speech=args.musan_snr_speech,
        musan_snr_music=args.musan_snr_music,
    ...
    if args.monitor_trials:
        print(f"Monitor every epochs: {max(int(args.monitor_every_epochs), 1)}")
    if args.train_crop_mode == "active":
        print("Train crop mode: active")
    if args.musan_root:
        musan_counts = {
            key: len(value) for key, value in train_dataset.musan_files.items()
        }
        print(
            f"MUSAN augmentation: prob={args.musan_prob:.2f} "
            f"counts={musan_counts} "
            f"snr_noise={args.musan_snr_noise} "
            f"snr_speech={args.musan_snr_speech} "
            f"snr_music={args.musan_snr_music}"
        )
    if args.rir_root:
        print(
            f"RIR augmentation: prob={args.reverb_prob:.2f} "
            f"count={len(train_dataset.rir_files)}"
        )
...
def main():
    ...
    if any(monitor_fields) and not (
        bool(args.monitor_base_path) and bool(args.monitor_trials)
    ):
        raise ValueError(
            "--monitor-base-path and --monitor-trials must be provided together. "
            "--monitor-enroll-list is optional for official three-column trial files."
        )
    if args.monitor_base_path and args.monitor_trials:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        veri_metrics = None
        monitor_interval = max(int(args.monitor_every_epochs), 1)
        should_run_monitor = bool(monitor_trials) and (
            (epoch % monitor_interval == 0) or (epoch == args.epochs)
        )
        ...
        if veri_metrics is not None:
            ...
            summary += f" | val EER: skipped (every {monitor_interval} epochs)"
        summary += (
        ...
        metrics_row = {
            ...
            "selected_metric": (float(metric) if metric is not None else None),
